module commondata
        ...
        ! Grid parameters
        integer(kind=4), parameter :: nx=2049, ny=nx
        ...
        integer(kind=4), parameter :: itc_max = 5 ! Maximum iterations
...
subroutine initial()
    ...
    do i = 1, nx
        if(i == 1)then
            interval(i,:) = (/i+1, i, i+2/)

        elseif(i == nx)then
            interval(i,:) = (/i-1, i, i-2/)

        else
            interval(i,:) = (/i-1, i, i+1/)
        end if
    enddo
...
program main
    ...
    !$acc data copy(u,v,rho,temp) copyin(xGrid,yGrid,ex,ey,omega_U,omega_T,f,g,interval)&
...
!$acc parallel loop default(none) present(f,f_post)  gang vector collapse(2) async(1)
    do j = 1, ny
        do i = 1, nx
...
!$acc parallel loop default(none) present(g,g_post)  gang vector collapse(2) async(2)
    do j = 1, ny
        do i = 1, nx
...
!$acc parallel loop present(f,f_post,ex,ey,xGrid,yGrid) gang vector collapse(2) async(1)
        do j = 1, ny
            ...
            f0 = interpolateF(yGrid(interval(j,1))+delta_y, yGrid(interval(j,2))+delta_y, yGrid(interval(j,3))+delta_y&
                , yGrid(interval(j,2)), f_post(interval(i,1), interval(j,1), alpha), f_post(interval(i,1), interval(j,2)&
                , alpha), f_post(interval(i,1), interval(j,3), alpha))

            f1 = interpolateF(yGrid(interval(j,1))+delta_y, yGrid(interval(j,2))+delta_y, yGrid(interval(j,3))+delta_y&
                , yGrid(interval(j,2)), f_post(interval(i,2), interval(j,1), alpha), f_post(interval(i,2), interval(j,2)&
                , alpha), f_post(interval(i,2), interval(j,3), alpha))

            f2 = interpolateF(yGrid(interval(j,1))+delta_y, yGrid(interval(j,2))+delta_y, yGrid(interval(j,3))+delta_y&
                , yGrid(interval(j,2)), f_post(interval(i,3), interval(j,1), alpha), f_post(interval(i,3), interval(j,2)&
                , alpha), f_post(interval(i,3), interval(j,3), alpha))

            f(i, j, alpha) = interpolateF(xGrid(interval(i,1))+delta_x, xGrid(interval(i,2))+delta_x, &
                            xGrid(interval(i,3))+delta_x, xGrid(interval(i,2)), f0, f1, f2)
...
!$acc parallel loop present(g,g_post,ex,ey,xGrid,yGrid) gang vector collapse(2) async(2)
        do j = 1, ny
            ...
            g0 = interpolateF(yGrid(interval(j,1))+delta_y, yGrid(interval(j,2))+delta_y, yGrid(interval(j,3))+delta_y&
                , yGrid(interval(j,2)), g_post(interval(i,1), interval(j,1), alpha), g_post(interval(i,1), interval(j,2)&
                , alpha), g_post(interval(i,1), interval(j,3), alpha))

            g1 = interpolateF(yGrid(interval(j,1))+delta_y, yGrid(interval(j,2))+delta_y, yGrid(interval(j,3))+delta_y&
                , yGrid(interval(j,2)), g_post(interval(i,2), interval(j,1), alpha), g_post(interval(i,2), interval(j,2)&
                , alpha), g_post(interval(i,2), interval(j,3), alpha))

            g2 = interpolateF(yGrid(interval(j,1))+delta_y, yGrid(interval(j,2))+delta_y, yGrid(interval(j,3))+delta_y&
                , yGrid(interval(j,2)), g_post(interval(i,3), interval(j,1), alpha), g_post(interval(i,3), interval(j,2)&
                , alpha), g_post(interval(i,3), interval(j,3), alpha))

            g(i, j, alpha) = interpolateF(xGrid(interval(i,1))+delta_x, xGrid(interval(i,2))+delta_x, &
                            xGrid(interval(i,3))+delta_x, xGrid(interval(i,2)), g0, g1, g2)
                enddo
            end do
        end do
!$acc end parallel loop
